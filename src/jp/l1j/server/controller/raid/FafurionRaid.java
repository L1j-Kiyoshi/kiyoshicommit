...
public class FafurionRaid extends DragonRaid {
	...
	private static Logger _log = Logger.getLogger(FafurionRaid.class.getName());

	public FafurionRaid(int mapid) {
		super();
		_mapId = mapid;
		_raidConfig = RaidConfigTable.getInstance().getRaidConfig(FAFURION_RAID);
		_allDropList = RaidDropTable.getInstance().getRaidDrop(FAFURION_RAID);
		MAX_DROP_PATTERN = RaidDropTable.getInstance().getMaxDropPattern(FAFURION_RAID);
		setDrop();
	}
		...
		if (round == 1 || round == 2) {
			...
			for (L1PcInstance pc : _dragonSlayers) {
				if (isDragonSlayer(pc)) {
					item = pc.getInventory().storeItem(50557, 1); // 水竜の証
				}
			...
			for (L1PcInstance pc2 : _dragonSlayers) {
				if (isDragonSlayer(pc2)) {
					item = pc2.getInventory().storeItem(50557, 1); // 水竜の証
				}
				for (L1PcInstance pclist : _dragonSlayers) {
					if (isDragonSlayer(pclist)) {
						pclist.sendPackets(new S_ServerMessage(813, _dragon.getNameId(),
	...
	private void addAntharasBloodstain() {
		for (L1PcInstance pc : _dragonSlayers) {
			if (isDragonSlayer(pc)) {
				L1BuffUtil.bloodstain(pc, (byte) 1, Config.DRAGON_SLAYER_INTERVAL, true); // パプリオンの血痕
			}
		}
	}
	...
	private void sendMessage(int round) {
		try {
			if (round == 1) {
				for (int msgid = 1657; msgid <= 1659; msgid++) {
					Thread.sleep(5000);
					for (L1PcInstance pc : _dragonSlayers) {
						if (isDragonSlayer(pc)) {
							pc.sendPackets(new S_ServerMessage(msgid));
				...
				Thread.sleep(5000);
				for (int msgid = 1668; msgid <= 1669; msgid++) {
					Thread.sleep(5000);
					for (L1PcInstance pc : _dragonSlayers) {
						if (isDragonSlayer(pc)) {
							pc.sendPackets(new S_ServerMessage(msgid));
	...
	private void spawnDragon(int round) {
		try {
			int[] npcId = new int[] { 91514, 91515, 91516 };
			...
			if (_dragon.getGfxId() == 7864 // パプリオンLv1
					|| _dragon.getGfxId() == 7869 // パプリオンLv2
					|| _dragon.getGfxId() == 7870 // パプリオンLv3
					) {
