...
public class ThebesRaidController extends Thread {

	private static Logger _log = Logger.getLogger(ThebesRaidController.class.getName());

	private static ThebesRaidController _instance = null;

	public static synchronized ThebesRaidController getInstance() {
		if (_instance == null) {
			_instance = new ThebesRaidController();
		}
		return _instance;
	}
	...
	private ThebesRaidController() {
		_raidConfig = RaidConfigTable.getInstance().getRaidConfig(THEBES_RAID);
		_allDropList = RaidDropTable.getInstance().getRaidDrop(THEBES_RAID);
		MAX_DROP_PATTERN = RaidDropTable.getInstance().getMaxDropPattern(THEBES_RAID);
	}
	...
	public void run() {
		try {
			...
			for (L1PcInstance pc : _playerList) {
				// オシリス： 愚かな霊魂どもめ！ここは貴様らごときがいてよい場所ではない。 アヌビス！ ホルス！ 奴らを片付けろ！
			...
			if (isBossEnd) { // ボス討伐に成功した
				checkPlayerList();
				for (L1PcInstance pc : _playerList) {
					// テーベ オシリス： なんということだ！ 我々が負けるとは…
					pc.sendPackets(new S_ServerMessage(1474));
				}
				Thread.sleep(5000);
				checkPlayerList();
				for (L1PcInstance pc : _playerList) {
					// テーベ オシリス： これから1日間テーベラスを開放する。
	...
	private void storeDrop() {
		...
		for (L1RaidDrop drop : _dropList) {
			...
			for (L1PcInstance pclist : _playerList) {
				pclist.sendPackets(new S_ServerMessage(813, "$8318", item.getLogName(), pc.getName()));
			}
		}
	}

	private void spawnHors() {
		_boss1 = NpcTable.getInstance().newNpcInstance(46123); // ホルス
		_boss1.setId(IdFactory.getInstance().nextId());
		_boss1.setMap((short) 782);
		_boss1.getLocation().set(32782, 32836, 782);
	...
	private void spawnAnubis() {
		_boss2 = NpcTable.getInstance().newNpcInstance(46124); // アヌビス
		_boss2.setId(IdFactory.getInstance().nextId());
		_boss2.setMap((short) 782);
		_boss2.getLocation().set(32782, 32827, 782);
