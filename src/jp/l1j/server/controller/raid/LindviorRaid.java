...
public class LindviorRaid extends DragonRaid {

	private static Logger _log = Logger.getLogger(LindviorRaid.class.getName());

	public LindviorRaid(int mapid) {
		super();
		_mapId = mapid;
		_raidConfig = RaidConfigTable.getInstance().getRaidConfig(LINDVIOR_RAID);
		_allDropList = RaidDropTable.getInstance().getRaidDrop(LINDVIOR_RAID);
		MAX_DROP_PATTERN = RaidDropTable.getInstance().getMaxDropPattern(LINDVIOR_RAID);
		setDrop();
	}
		...
		if (round == 1 || round == 2) {
			...
			for (L1PcInstance pc : _dragonSlayers) {
				if (isDragonSlayer(pc)) {
					item = pc.getInventory().storeItem(50558, 1); // 風竜の証
				}
			...
			for (L1PcInstance pc2 : _dragonSlayers) {
				if (isDragonSlayer(pc2)) {
					item = pc2.getInventory().storeItem(50558, 1); // 水竜の証
				}
				for (L1PcInstance pclist : _dragonSlayers) {
					if (isDragonSlayer(pclist)) {
						pclist.sendPackets(new S_ServerMessage(813, _dragon.getNameId(),
	...
	private void addAntharasBloodstain() {
		for (L1PcInstance pc : _dragonSlayers) {
			if (isDragonSlayer(pc)) {
				L1BuffUtil.bloodstain(pc, (byte) 2, Config.DRAGON_SLAYER_INTERVAL, true); // リンドビオルの血痕
			}
		}
	}
	...
	private boolean isDragonSlayer(L1PcInstance pc) {
		...
		if (pc.getX() < 32928 || 32993 < pc.getX()) {
			return false;
		}
		if (pc.getY() < 32812 || 32860 < pc.getY()) {
			return false;
		}
		return true;
	}
	...
	private void sendMessage(int round) {
		try {
			if (round == 1) {
				for (int msgid = 1755; msgid <= 1759; msgid++) {
					Thread.sleep(5000);
					for (L1PcInstance pc : _dragonSlayers) {
						if (isDragonSlayer(pc)) {
							pc.sendPackets(new S_ServerMessage(msgid));
				...
				for (L1PcInstance pc : _dragonSlayers) {
					if (isDragonSlayer(pc)) {
						pc.sendPackets(new S_ServerMessage(1761));
					}
				}
				Thread.sleep(60000);
				for (L1PcInstance pc : _dragonSlayers) {
					if (isDragonSlayer(pc)) {
						pc.sendPackets(new S_ServerMessage(1762));
					}
				}
			} else if (round == 3) {
				for (L1PcInstance pc : _dragonSlayers) {
					if (isDragonSlayer(pc)) {
						pc.sendPackets(new S_ServerMessage(1764));
					}
				}
				Thread.sleep(60000);
				for (L1PcInstance pc : _dragonSlayers) {
					if (isDragonSlayer(pc)) {
						pc.sendPackets(new S_ServerMessage(1765));
					}
				}
			} else if (round == 4) {
				Thread.sleep(5000);
				for (int msgid = 1772; msgid <= 1773; msgid++) {
					Thread.sleep(5000);
					for (L1PcInstance pc : _dragonSlayers) {
						if (isDragonSlayer(pc)) {
							pc.sendPackets(new S_ServerMessage(msgid));
	...
	private void spawnDragon(int round) {
		try {
			int[] npcId = new int[] { 91603, 91604, 91605 };
			...
			if (round != 2) {
				...
				do {
					tryCount++;
					_dragon.setX(32955 + _random.nextInt(10) - _random.nextInt(10));
					_dragon.setY(32839 + _random.nextInt(10) - _random.nextInt(10));
			...
			if (_dragon.getGfxId() == 8036 // リンドビオルLv1
					|| _dragon.getGfxId() == 8056 // リンドビオルLv2
					|| _dragon.getGfxId() == 8055 // リンドビオルLv3
					) {
