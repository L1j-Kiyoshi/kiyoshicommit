	...
	public C_Door(byte abyte0[], ClientThread client)
		...
		if (npcId == 71254) { // 時の歪み
			if (Config.DISTORTIONS) {
				L1Location teleportLoc =
					DistortionOfTimeController.getInstance().getTeleportLocation();
				L1Teleport.teleport(pc, teleportLoc, 5, false);
			} else {
				L1Location[] teleportLoc = DistortionOfTimeController.getInstance()
												.getTeleportLocationList();
				L1Teleport.teleport(pc, teleportLoc[_random.nextInt(teleportLoc.length)], 5, false);
			}
		}
