...
public class C_LoginToServer extends ClientBasePacket {
	...
	public C_LoginToServer(byte abyte0[], ClientThread client)
		...
		MapTimerTable.getInstance().loadMapTimer(pc); // マップリミッターロード
		sendConfigLog(pc);
	}

	private void sendConfigLog(L1PcInstance pc) {
		// Config値から各設定を取得し表示する。
		StringBuffer config = new StringBuffer("PotLog:");
		config.append(pc.isPotLog() ? "有" : "無");
		config.append("  DropLog:");
		config.append(pc.isDropLog() ? "有" : "無");
		config.append("  PartyDropLog:");
		config.append(pc.isPartyDropLog() ? "有" : "無");
		StringBuffer config2 = new StringBuffer("getAlignment:");
		config2.append(pc.isAlignmentGet() ? "有" : "無");
		pc.sendPackets(new S_SystemMessage(config.toString()));
		pc.sendPackets(new S_SystemMessage(config2.toString()));
	}
	...
	private void buff(ClientThread clientthread, L1PcInstance pc) {
		for (L1CharacterBuff buff : CharBuffTable.findByCharacterId(pc.getId())) {
			...
			if (skillId == SHAPE_CHANGE) { // 変身
				...
				pc.setSkillEffect(skillId, remainingTime * 1000);
				pc.setBraveSpeed(1);
			} else if (skillId == CUBE_IGNITION_ALLY) { // キューブ[イグニション]
				pc.addFire(30);
				pc.sendPackets(new S_OwnCharAttrDef(pc));
				pc.setSkillEffect(skillId, remainingTime * 1000);
			} else if (skillId == CUBE_QUAKE_ALLY) { // キューブ[クエイク]
				pc.addEarth(30);
				pc.sendPackets(new S_OwnCharAttrDef(pc));
				pc.setSkillEffect(skillId, remainingTime * 1000);
			} else if (skillId == CUBE_SHOCK_ALLY) { // キューブ[ショック]
				pc.addWind(30);
				pc.sendPackets(new S_OwnCharAttrDef(pc));
				pc.setSkillEffect(skillId, remainingTime * 1000);
				...
				pc.addStr(6);
				pc.sendPackets(new S_Strup(pc, 6, remainingTime));
				pc.setSkillEffect(skillId, remainingTime * 1000);
			} else if (skillId == STATUS_FLORA_POTION_DEX) { // 才能、フローラポーション
				pc.addDex(6);
				pc.sendPackets(new S_Dexup(pc, 6, remainingTime));
				pc.setSkillEffect(skillId, remainingTime * 1000);
			} else if (skillId == STATUS_FLORA_POTION_AC) { // 才能、フローラポーション(AC)
				pc.addAc(-5);
				pc.sendPackets(new S_SkillIconShield(5, remainingTime));
				pc.setSkillEffect(skillId, remainingTime * 1000);
			} else if (skillId == EARTH_BIND) { // 凍結
				pc.sendPackets(new S_Paralysis(S_Paralysis.TYPE_BIND, true));
				pc.setSkillEffect(skillId, remainingTime * 1000);
			} else if (skillId == ICE_LANCE) { // 凍結
				pc.sendPackets(new S_Paralysis(S_Paralysis.TYPE_BIND, true));
				L1EffectSpawn.getInstance().spawnEffect(81168, remainingTime * 1000,
				...
				pc.addHitup(potion.getEffect(pc, false).getHit());
				pc.addDmgup(potion.getEffect(pc, false).getDmg());
				pc.addBowHitup(potion.getEffect(pc, false).getBowHit());
				pc.addBowDmgup(potion.getEffect(pc, false).getBowDmg());
				pc.addSp(potion.getEffect(pc, false).getSp());
				pc.sendPackets(new S_SpMr(pc));
				pc.setSkillEffect(skillId, remainingTime * 1000);
