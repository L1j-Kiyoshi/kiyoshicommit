...
import jp.l1j.server.datatables.BeginnerItemsTable;
...
public class C_CreateChar extends ClientBasePacket {
	...
	private static void giveBeginnerItems(L1PcInstance pc) {
		String classNameInitial = pc.getClassFeature().getClassNameInitial();
		boolean isStore;
		for (L1BeginnerItems items : BeginnerItemsTable.getInstance().getAllBeginnerItems()) {
			isStore = false;
			for (String classInitial : items.getClassInitial()) {
				if (classInitial.equalsIgnoreCase("A") || classInitial.equalsIgnoreCase(classNameInitial)) {
					isStore = true;
					break;
				}
			}
			...
			L1Item temp = ItemTable.getInstance().getTemplate(items.getItemId());
			if (temp != null) {
				if (temp.isStackable()) {
					L1ItemInstance item = ItemTable.getInstance().createItem(items.getItemId());
					item.setEnchantLevel(items.getEnchantLevel());
					item.setCount(items.getItemCount());
					...
					int createCount;
					for (createCount = 0; createCount < items.getItemCount(); createCount++) {
