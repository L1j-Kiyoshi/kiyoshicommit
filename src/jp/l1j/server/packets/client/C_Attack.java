	...
	public C_Attack(byte[] decrypt, ClientThread client) {
		...
		if (target != null && !((L1Character) target).isDead()) {
			...
			if (weapon != null) {
				...
				weaponType2 = weapon.getItem().getType();
				sayhagfx = weapon.getItem().getSayhaGfx();
				if (weaponType == 20) { // 弓
			...
			if (weaponType == 20 && (sayhagfx != -1 || arrow != null)) {
				...
				} else if (sayhagfx != -1) { // /L1J2/src/l1j/server/server/clientpackets/C_Attack.java
					pc.sendPackets(new S_UseArrowSkill(pc, 0, sayhagfx, _targetX, _targetY, true));
					pc.broadcastPacket(new S_UseArrowSkill(pc, 0, sayhagfx, _targetX, _targetY, true));
				}
			...
			} else if (weaponType2 == 14) { // キーリンク
				int castgfx = SkillTable.getInstance().findBySkillId(weapon.getItem().getWeaponSpecial()).getCastGfx();
