...
import jp.l1j.server.model.item.executor.L1DragonKey;
...
public class C_UseItem extends ClientBasePacket {
	...
	public C_UseItem(byte abyte0[], ClientThread client) throws Exception {
			...
			if (delayEffect > 0) {
				...
				if (lastUsed != null) {
					...
					if ((cal.getTimeInMillis() - lastUsed.getTime()) / 1000 <= delayEffect) {
						long nextUseTime = lastUsed.getTime() + (delayEffect * 1000);
						long delayTime = ((nextUseTime - cal.getTimeInMillis()) / 1000) / 60;
						Calendar now = Calendar.getInstance();
						now.setTimeInMillis(nextUseTime);
						String nextTime = now.get(Calendar.HOUR_OF_DAY) + ":" + now.get(Calendar.MINUTE);
						//S_ServerMessageでは引数が%dの場合表示されない。
						pc.sendPackets(new S_SystemMessage(String.format(I18N_CANT_BE_USED_TO_DATE,
																	String.valueOf(delayTime), nextTime)));
						 // %d分間(%sまで)は使用できません。
						//pc.sendPackets(new S_ServerMessage(3306, nextTime, String.valueOf(delayTime)));
						return;
					}
				}
			}
		...
		if (item.getItem().getType2() == 0) { // 種別：その他のアイテム
			...
			if (itemId == 40003) { // ランタン オイル
			...
			} else if (itemId == 41541) { // 強化のチャーム
				int[] skills = { PHYSICAL_ENCHANT_STR, PHYSICAL_ENCHANT_DEX, BLESS_WEAPON };
				for (int skillId : skills) {
					new L1SkillUse().handleCommands(pc ,skillId,
							pc.getId(), pc.getX(), pc.getY(), null, 1200, L1SkillUse.TYPE_GMBUFF);
				}
				...
				pc.getInventory().removeItem(item, 1);
			} else if (41504 <= itemId && itemId <= 41509) { // 魔族の変換石各種
				L1ItemInstance target = pc.getInventory().getItem(objid);
				...
				if (itemId == 41504 && target.getItemId() == 281) { // ソード変換石
					isChange = true;
				} else if (itemId == 41505 && target.getItemId() == 280) { // ボウ変換石
					isChange = true;
				} else if (itemId == 41506 && target.getItemId() == 279) { // スタッフ変換石
					isChange = true;
				} else if (itemId == 41507 && target.getItemId() == 278) { // クロウ変換石
					isChange = true;
				} else if (itemId == 41508 && target.getItemId() == 277) { // チェーンソード変換石
					isChange = true;
				} else if (itemId == 41509 && target.getItemId() == 276) { // キーリンク変換石
					isChange = true;
				}
				if (isChange) {
					L1ItemInstance createItem = ItemTable.getInstance().createItem(target.getItemId() + 100);
					createItem.setAttrEnchantKind(target.getAttrEnchantKind());
					createItem.setAttrEnchantLevel(target.getAttrEnchantLevel());
					createItem.setEnchantLevel(target.getEnchantLevel());
					pc.getInventory().removeItem(target);
					pc.getInventory().removeItem(item, 1);
					pc.getInventory().storeItem(createItem);
					pc.sendPackets(new S_ServerMessage(403, createItem.getItem().getUnidentifiedNameId()));
				} else {
					pc.sendPackets(new S_ServerMessage(79));
					// \f1何も起きませんでした。
				}
			} else if (itemId == 41545) { // 士魂
				L1ItemInstance target = pc.getInventory().getItem(objid);
				if (target.getItemId() == 41542) { // 天狗の羽団扇
					L1ItemInstance createItem = ItemTable.getInstance().createItem(382);
					pc.getInventory().removeItem(target);
					pc.getInventory().removeItem(item, 1);
					pc.getInventory().storeItem(createItem);
					pc.sendPackets(new S_ServerMessage(403, createItem.getItem().getUnidentifiedNameId()));
			...
			} else if (itemId == 50501 || (41520 <= itemId && itemId <= 41523)) { // ドラゴンキー
				L1DragonKey.getInstance().useKey(pc, item);
				...
				if (box != null) {
					if (box.open(pc, item)) {
						...
						if (item.getChargeCount() > 1) { // 使用時に２チャージ以上の場合は削除しない
							item.setChargeCount(item.getChargeCount() - 1);
							pc.getInventory().updateItem(item, L1PcInventory.COL_CHARGE_COUNT);
						} else if (isDelayEffect && item.getChargeCount() == 1) {
							pc.getInventory().deleteItem(item.getId(), item);
						} else if (!isDelayEffect && item.getChargeCount() == 0) { // ディレイがなく、チャージカウントもない場合は削除
	...
	private void UseArmor(L1PcInstance activeChar, L1ItemInstance armor) {
		...
		if (type == 11) { // リングの場合
			if (activeChar.isRingSlotOpen81()) { // 81開放済
				equipeSpace = pcInventory.getTypeEquipped(2, type) <= 3;
			} else if (activeChar.isRingSlotOpen76()) { // 76開放済
				equipeSpace = pcInventory.getTypeEquipped(2, type) <= 2;
			} else { // 未開放
		...
		if (equipeSpace && !armor.isEquipped()) {
			...
			if (type == 11) { // リングで同種を３つ以上は装備不可
				if (activeChar.getInventory().checkEquipped(armor.getItemId(), 2)) {
					activeChar.sendPackets(new S_ServerMessage(3278));
					// \f1同じ種類のリングはこれ以上装着できません。
					return;
				}
			}

			if (type == 7) { // シールドの場合、両手武器を装備していると装備不可
				if (activeChar.getWeapon() != null) {
					if (activeChar.getWeapon().getItem().isTwohanded()) { // 両手武器を装備
						activeChar.sendPackets(new S_ServerMessage(129));
						return;
						// \f1両手の武器を武装したままシールドを着用することはできません。
					}
				}
				if (activeChar.getInventory().getTypeEquipped(2, 8) >= 1) { // ガーダー装備中
					activeChar.sendPackets(new S_ServerMessage(124));
					// \f1すでに何かを装備しています。
					return;
				}
			}
			if (type == 8) { // ガーダーの場合、シールド装備不可
				if (activeChar.getInventory().getTypeEquipped(2, 7) >= 1) { // シールド装備
					activeChar.sendPackets(new S_ServerMessage(124));
					// \f1すでに何かを装備しています。
					return;
				}
	...
	private void UseWeapon(L1PcInstance activeChar, L1ItemInstance weapon) {
		...
		if (activeChar.getWeapon() != null) { // 既に何かを装備している場合、前の装備をはずす
			if (activeChar.getWeapon().getItem().getBless() == 2) { // 呪われていた場合
				activeChar.sendPackets(new S_ServerMessage(150)); // はずすことができません。呪いをかけられているようです。
				return;
			}
			if (activeChar.getWeapon().equals(weapon)) { // 装備交換ではなく外すだけ
				if (activeChar.hasSkillEffect(DANCING_BLAZE)) {
					activeChar.removeSkillEffect(DANCING_BLAZE);
				}
