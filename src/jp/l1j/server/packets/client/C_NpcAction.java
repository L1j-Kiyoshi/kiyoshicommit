...
import jp.l1j.server.controller.timer.UltimateBattleTimeController;
...
public class C_NpcAction extends ClientBasePacket {
	...
	public C_NpcAction(byte abyte0[], ClientThread client) throws Exception {
		...
		} else if (npcTemp.getNpcId() == 91328) {
		...
		} else if (npcTemp.getNpcId() == 91329) {
		...
		// 敵対的なアイスクイーンの近衛兵
		else if (npcTemp.getNpcId() == 71274) {
			if (s.equalsIgnoreCase("enter")) { // グッとこらえて移動する
				IceQueenCastleController iqcc = new IceQueenCastleController(pc, 1);
				L1Teleport.teleport(pc, 32728, 32819, iqcc.getMapId(), 0, true);
				GeneralThreadPool.getInstance().execute(iqcc);
				htmlid = "";
			}

		}
		// 友好的なアイスクイーンの近衛兵
		else if (npcTemp.getNpcId() == 71275) {
			if (s.equalsIgnoreCase("enter")) { // 侍女たちを追い払うので城内に移動させてください。
				IceQueenCastleController iqcc = new IceQueenCastleController(pc, 2);
				L1Teleport.teleport(pc, 32728, 32819, iqcc.getMapId(), 0, true);
				GeneralThreadPool.getInstance().execute(iqcc);
				htmlid = "";
			}
		...
		} else if (npcTemp.getNpcId() == 80167) { // 善の使者
			// ミリエル
		...
		// 長老 ノナメ
		else if (npcTemp.getNpcId() == 71038) {
			// 「手紙を受け取る」
		...
		// ネルガ フウモ
		else if (npcTemp.getNpcId() == 71041) {
			// 「調査をします」
			if (s.equalsIgnoreCase("A")) {
				npc = (L1NpcInstance) obj;
		...
		// ネルガ バクモ
		else if (npcTemp.getNpcId() == 71042) {
			// 「調査をします」
			if (s.equalsIgnoreCase("A")) {
				npc = (L1NpcInstance) obj;
		...
		// ドゥダ-マラ ブカ
		else if (npcTemp.getNpcId() == 71043) {
			// 「調査をします」
			if (s.equalsIgnoreCase("A")) {
				npc = (L1NpcInstance) obj;
		...
		// ドゥダ-マラ カメ
		else if (npcTemp.getNpcId() == 71044) {
			// 「調査をします」
			if (s.equalsIgnoreCase("A")) {
				npc = (L1NpcInstance) obj;
		...
		// 空間の歪み
		else if (npcTemp.getNpcId() == 80048) {
			// 「やめる」
		...
		// デジェネレイト ソウル
		else if (npcTemp.getNpcId() == 71095) {
		...
		// 揺らぐ者
		else if (npcTemp.getNpcId() == 80049) {
			// 「バルログの意志を迎え入れる」
		...
		// ヤヒの執政官
		else if (npcTemp.getNpcId() == 80050) {
			// 「私の霊魂はヤヒ様へ…」
			if (s.equalsIgnoreCase("1")) {
				htmlid = "meet105";
			}
		...
		// ヤヒの軍師
		else if (npcTemp.getNpcId() == 80052) {
			// 私に力をくださいますよう・・・
			if (s.equalsIgnoreCase("a")) {
		...
		// ヤヒの鍛冶屋
		else if (npcTemp.getNpcId() == 80053) {
			// 「材料すべてを用意できました」
			if (s.equalsIgnoreCase("a")) {
		...
		// ヤヒの補佐官
		else if (npcTemp.getNpcId() == 80055) {
			npc = (L1NpcInstance) obj;
			htmlid = getYaheeAmulet(pc, npc, s);
		}
		// 業の管理者
		else if (npcTemp.getNpcId() == 80056) {
			npc = (L1NpcInstance) obj;
			if (pc.getKarma() <= -10000000) {
		...
		// 次元の扉(バルログの部屋)
		else if (npcTemp.getNpcId() == 80063) {
			// 「中に入る」
			if (s.equalsIgnoreCase("a")) {
		...
		// バルログの執政官
		else if (npcTemp.getNpcId() == 80064) {
			// 「私の永遠の主はバルログ様だけです…」
			if (s.equalsIgnoreCase("1")) {
				htmlid = "meet005";
			}
		...
		// 揺らめく者
		else if (npcTemp.getNpcId() == 80066) {
			// 「カヘルの意志を受け入れる」
			if (s.equalsIgnoreCase("1")) {
				if (pc.getKarma() >= 10000000) {
					pc.setKarma(-1000000);
		...
		// バルログの補佐官
		else if (npcTemp.getNpcId() == 80071) {
			npc = (L1NpcInstance) obj;
			htmlid = getBarlogEarring(pc, npc, s);
		}
		// バルログの軍師
		else if (npcTemp.getNpcId() == 80073) {
			// 私に力をくださいますよう・・・
			if (s.equalsIgnoreCase("a")) {
		...
		// バルログの鍛冶屋
		else if (npcTemp.getNpcId() == 80072) {
			String sEquals = null;
			int karmaLevel = 0;
			int[] material = null;
			int[] count = null;
		...
		// 業の管理者
		else if (npcTemp.getNpcId() == 80074) {
			npc = (L1NpcInstance) obj;
			if (pc.getKarma() >= 10000000) {
		...
		// アルフォンス
		else if (npcTemp.getNpcId() == 80057) {
			htmlid = karmaLevelToHtmlId(pc.getKarmaLevel());
			htmldata = new String[] { String.valueOf(pc.getKarmaPercent()) };
		}
		// 次元の扉(土風水火)
		else if (npcTemp.getNpcId() == 80059
				|| npcTemp.getNpcId() == 80060
				|| npcTemp.getNpcId() == 80061
				|| npcTemp.getNpcId() == 80062) {
			htmlid = talkToDimensionDoor(pc, (L1NpcInstance) obj, s);
		}
		// ジャック オ ランタン
		else if (npcTemp.getNpcId() == 81124) {
			if (s.equalsIgnoreCase("1")) {
				poly(client, 4002);
				htmlid = ""; // ウィンドウを消す
			} else if (s.equalsIgnoreCase("2")) {
		...
		// タウンアドバイザー
		else if (npcTemp.getNpcId() == 70534
				|| npcTemp.getNpcId() == 70556
				|| npcTemp.getNpcId() == 70572
				|| npcTemp.getNpcId() == 70631
				|| npcTemp.getNpcId() == 70663
				|| npcTemp.getNpcId() == 70761
				|| npcTemp.getNpcId() == 70788
				|| npcTemp.getNpcId() == 70806
				|| npcTemp.getNpcId() == 70830
				|| npcTemp.getNpcId() == 70876) {
			L1TownAdvisorInstance advisor =
				new L1TownAdvisorInstance(npcTemp);
			advisor.create(pc, s);
			htmlid = "";
		}
		// ドロモンド
		else if (npcTemp.getNpcId() == 70997) {
			// ありがとう、旅立ちます
			if (s.equalsIgnoreCase("0")) {
		...
		// アレックス(歌う島)
		else if (npcTemp.getNpcId() == 70999) {
			// ドロモンドの紹介状を渡す
			if (s.equalsIgnoreCase("1")) {
		...
		// ポピレア(歌う島)
		else if (npcTemp.getNpcId() == 71005) {
			// アイテムを受け取る
			if (s.equalsIgnoreCase("0")) {
		...
		// ティミー(歌う島)
		else if (npcTemp.getNpcId() == 71006) {
			if (s.equalsIgnoreCase("0")) {
				if (pc.getLevel() > 25) {
					htmlid = "jpe0057";
		...
		else if (npcTemp.getNpcId() == 71029) {
		...
		// 治療師（歌う島の中：ＨＰのみ回復）
		else if (npcTemp.getNpcId() == 70512) {
		...
		// 治療師（訓練場：HPMP回復）
		else if (npcTemp.getNpcId() == 71037) {
		...
		// 治療師（西部）
		else if (npcTemp.getNpcId() == 71030) {
		...
		// キャンセレーション師
		else if (npcTemp.getNpcId() == 71002) {
			// キャンセレーション魔法をかけてもらう
			if (s.equalsIgnoreCase("0")) {
				if (pc.getLevel() <= 13) {
		...
		// ケスキン(歌う島)
		else if (npcTemp.getNpcId() == 71025) {
			if (s.equalsIgnoreCase("0")) {
		...
		// ルケイン(海賊島)
		else if (npcTemp.getNpcId() == 71055) {
			// アイテムを受け取る
			if (s.equalsIgnoreCase("0")) {
		...
		// 小さな箱-1番目
		else if (npcTemp.getNpcId() == 71063) {
			if (s.equalsIgnoreCase("0")) {
		...
		// 小さな箱-2番目
		else if (npcTemp.getNpcId() == 71064
				|| npcTemp.getNpcId() == 71065
				|| npcTemp.getNpcId() == 71066) {
			if (s.equalsIgnoreCase("0")) {
				materials = new int[] { 40701 }; // 小さな宝の地図
				counts = new int[] { 1 };
				createitem = new int[] { 40702 }; // 小さな袋
		...
		// シミズ(海賊島)
		else if (npcTemp.getNpcId() == 71056) {
			// 息子を捜す
			if (s.equalsIgnoreCase("a")) {
		...
		// ドイル(海賊島)
		else if (npcTemp.getNpcId() == 71057) {
			// ラッシュについて聞く
		...
		// ルディアン(海賊島)
		else if (npcTemp.getNpcId() == 71059) {
			// ルディアンの頼みを受け入れる
			if (s.equalsIgnoreCase("A")) {
				htmlid = "rudian6";
		...
		// レスタ(海賊島)
		else if (npcTemp.getNpcId() == 71060) {
			// 仲間たちについて
			if (s.equalsIgnoreCase("A")) {
		...
		// カドムス(海賊島)
		else if (npcTemp.getNpcId() == 71061) {
			// 地図を組み合わせてください
			if (s.equalsIgnoreCase("A")) {
		...
		// カミーラ(海賊島)
		else if (npcTemp.getNpcId() == 71036) {
			if (s.equalsIgnoreCase("a")) {
				htmlid = "kamyla7";
		...
		// フランコ(海賊島)
		else if (npcTemp.getNpcId() == 71089) {
			// カミーラにあなたの潔白を証明しましょう
			if (s.equalsIgnoreCase("a")) {
				htmlid = "francu10";
		...
		// 試練のクリスタル2(海賊島)
		else if (npcTemp.getNpcId() == 71090) {
			// はい、武器とスクロールをください
			if (s.equalsIgnoreCase("a")) {
				htmlid = "";
		...
		// 試練のクリスタル2(海賊島)
		else if (npcTemp.getNpcId() == 71091) {
			// さらば！！
			if (s.equalsIgnoreCase("a")) {
				htmlid = "";
		...
		// リザードマンの長老(海賊島)
		else if (npcTemp.getNpcId() == 71074) {
			// その戦士は今どこらへんにいるんですか？
			if (s.equalsIgnoreCase("A")) {
				htmlid = "lelder5";
		...
		// 傭兵団長 ティオン
		else if (npcTemp.getNpcId() == 71198) {
			if (s.equalsIgnoreCase("A")) {
		...
		// ジェロン
		else if (npcTemp.getNpcId() == 71199) {
			if (s.equalsIgnoreCase("A")) {
		...
		// 占星術師ケプリシャ
		else if (npcTemp.getNpcId() == 80079) {
			// ケプリシャと魂の契約を結ぶ
			if (s.equalsIgnoreCase("0")) {
		...
		// 釣りっ子(IN)
		else if (npcTemp.getNpcId() == 80082) {
			// 「魚釣りをする」
			if (s.equalsIgnoreCase("a")) {
				if (pc.getLevel() < 15) {
		...
		// 釣りっ子(OUT)
		else if (npcTemp.getNpcId() == 80083) {
			// 「外に出る」
		...
		// 怪しいオーク商人 パルーム
		else if (npcTemp.getNpcId() == 80084) {
			// 「資源リストをもらう」
		...
		// アデン騎馬団員
		else if (npcTemp.getNpcId() == 80105) {
			// 「新たな力をくださいる」
		...
		// 補佐官イリス
		else if (npcTemp.getNpcId() == 71126) {
			if (s.equalsIgnoreCase("0") || s.equalsIgnoreCase("1") || // 中央広場、 冥法軍入口付近
					s.equalsIgnoreCase("2") || s.equalsIgnoreCase("3") || // 魔霊軍入口付近、魔獣軍入口付近
					s.equalsIgnoreCase("4")) { // 暗殺軍入口付近へ移動
				int[] locX = { 32840, 32788, 32913, 32907, 32737 };
				int[] locY = { 32766, 32674, 32696, 32813, 32833 };
				int newX = locX[Integer.parseInt(s)] + _random.nextInt(9);
				int newY = locY[Integer.parseInt(s)] + _random.nextInt(9);
				L1Teleport.teleport(pc, newX, newY, (short) 479, 5, true);
			} else if (s.equalsIgnoreCase("B")) { // 「はい。私がご協力しましょう」
		...
		// 倒れた航海士
		else if (npcTemp.getNpcId() == 80076) {
			if (s.equalsIgnoreCase("A")) {
		...
		// 錬金術師 ペリター
		else if (npcTemp.getNpcId() == 71128) {
			if (s.equals("A")) {
				if (pc.getInventory().checkItem(41010, 1)) { // イリスの推薦書
					htmlid = "perita2";
				} else {
		...
		// 宝石細工師 ルームィス
		else if (npcTemp.getNpcId() == 71129) {
			if (s.equals("Z")) {
				htmlid = "rumtis2";
			} else if (s.equals("Y")) {
				if (pc.getInventory().checkItem(41010, 1)) { // イリスの推薦書
		...
		// アタロゼ
		else if (npcTemp.getNpcId() == 71119) {
		...
		// 長老随行員クロレンス
		else if (npcTemp.getNpcId() == 71170) {
			// 「ラスタバドの歴史書を渡す」
		...
		// 真冥王 ダンテス
		else if (npcTemp.getNpcId() == 71168) {
			// 「異界の魔物がいる場所へ送ってください」
			if (s.equalsIgnoreCase("a")) {
		...
		// 諜報員(欲望の洞窟側)
		else if (npcTemp.getNpcId() == 80067) {
			// 「動揺しつつも承諾する」
		...
		// 諜報員(影の神殿側)
		else if (npcTemp.getNpcId() == 81202) {
			// 「頭にくるが承諾する」
			if (s.equalsIgnoreCase("n")) {
				htmlid = "";
				poly(client, 6035);
		...
		// ゾウのストーンゴーレム
		else if (npcTemp.getNpcId() == 71252) {
			int weapon1 = 0;
			int weapon2 = 0;
			int newWeapon = 0;
			if (s.equalsIgnoreCase("A")) {
		...
		// ゾウのストーンゴーレム テーベ砂漠
		else if (npcTemp.getNpcId() == 71253) {
			// 「歪みのコアを作る」
			if (s.equalsIgnoreCase("A")) {
		...
		// テーベ オシリス祭壇のキーパー
		else if (npcTemp.getNpcId() == 71255) {
			L1RaidConfig raidConf = RaidConfigTable.getInstance().getRaidConfig(THEBES_RAID);
			// 「テーベオシリス祭壇の鍵を持っているなら、オシリスの祭壇にお送りしましょう。」
			if (s.equalsIgnoreCase("e")) {
				if (pc.getInventory().checkItem(49242, 1)) {
					if (!ThebesRaidController.getInstance().isEnterTime()) { // 入場時間外
						htmlid = "tebegate2";
						String times = Config.DISTORTION_RAID_OPEN_TIME + "分後";
						pc.sendPackets(new S_SystemMessage("時の歪みが出来て" + times + "に入場可能です。"));
					} else if (ThebesRaidController.getInstance().getPlayerList().size() >= raidConf.getMaxPlayer()) {
						htmlid = "tebegate4"; // 上限人数に達している
					} else if (raidConf.getMinLevel() != -1 && pc.getLevel() < raidConf.getMinLevel()) { // レイド参加条件（最低レベル以上でなければならない）
						pc.sendPackets(new S_SystemMessage("このレイドはレベル" + raidConf.getMinLevel()
															+ "以上でなければ参加できません。"));
					} else if (raidConf.getMaxLevel() != -1 && pc.getLevel() >= raidConf.getMaxLevel()) { // レイド参加条件（最高レベル未満でなければならない）
						pc.sendPackets(new S_SystemMessage("このレイドはレベル" + raidConf.getMaxLevel()
															+ "未満でなければ参加できません。"));
					} else {
						pc.getInventory().consumeItem(49242, 1);
						L1Teleport.teleport(pc, 32735, 32831, (short) 782, 2, true);
						ThebesRaidController.getInstance().addPlayerList(pc);
						htmlid = "";
					}
				} else {
					htmlid = "tebegate3";
		...
		// ククルカン祭壇のキーパー
		else if (npcTemp.getNpcId() == 90521) {
			L1RaidConfig raidConf = RaidConfigTable.getInstance().getRaidConfig(KUKULCAN_RAID);
			// 「ククルカン祭壇の鍵を持っているなら、そこまでお送りしましょう。」
			if (s.equalsIgnoreCase("e")) {
				if (pc.getInventory().checkItem(50005, 1)) {
					if (!KukulcanRaidController.getInstance().isEnterTime()) { // 入場時間外
						htmlid = ""; // 入場時間外のテキストがない
						String times = Config.DISTORTION_RAID_OPEN_TIME + "分後";
						pc.sendPackets(new S_SystemMessage("時の歪みが出来て" + times + "に入場可能です。"));
					} else if (KukulcanRaidController.getInstance().getPlayerList().size() >= raidConf.getMaxPlayer()) {
						htmlid = "tikalgate4"; // 上限人数に達している
					} else if (raidConf.getMinLevel() != -1 && pc.getLevel() < raidConf.getMinLevel()) { // レイド参加条件（最低レベル以上でなければならない）
						pc.sendPackets(new S_SystemMessage("このレイドはレベル" + raidConf.getMinLevel()
															+ "以上でなければ参加できません。"));
					} else if (raidConf.getMaxLevel() != -1 && pc.getLevel() >= raidConf.getMaxLevel()) { // レイド参加条件（最高レベル未満でなければならない）
						...
						KukulcanRaidController.getInstance().addPlayerList(pc);
						htmlid = "";
					}
				} else {
					htmlid = "tikalgate3";
		...
		// ロビンフッド
		else if (npcTemp.getNpcId() == 71256) {
			if (s.equalsIgnoreCase("E")) {
		...
		// ジブリル
		else if (npcTemp.getNpcId() == 71257) {
		...
		// マルバ
		else if (npcTemp.getNpcId() == 71258) {
		...
		// アラス
		else if (npcTemp.getNpcId() == 71259) {
			if (pc.getInventory().checkItem(40665)) {
				htmlid = "aras8";
		...
		// 治安団長ラルソン
		else if (npcTemp.getNpcId() == 80099) {
			if (s.equalsIgnoreCase("A")) {
		...
		// クエン
		else if (npcTemp.getNpcId() == 80101) {
		...
		// 長老 プロケル
		else if (npcTemp.getNpcId() == 80136) {
		...
		// 長老 シルレイン
		else if (npcTemp.getNpcId() == 80145) {
			int lv15_step = pc.getQuest().getStep(L1Quest.QUEST_LEVEL15);
			int lv30_step = pc.getQuest().getStep(L1Quest.QUEST_LEVEL30);
			int lv45_step = pc.getQuest().getStep(L1Quest.QUEST_LEVEL45);
			int lv50_step = pc.getQuest().getStep(L1Quest.QUEST_LEVEL50);
		...
		// エルラス
		else if (npcTemp.getNpcId() == 80135) {
			if (pc.isDragonKnight()) {
				// 「オーク密使変身スクロールを受け取る」
				if (s.equalsIgnoreCase("a")) {
		...
		else if (npcTemp.getNpcId() == 81245) {
			// オーク密使(HC3)
			if (pc.isDragonKnight()) {
		...
		else if (npcTemp.getNpcId() == 81246) { // シャルナ
			if (pc.getLevel() < 30) {
				htmlid = "sharna4";
				...
				createitem = new int[] { 50641 }; // シャルナの変身スクロール（レベル75）
			} else if (pc.getLevel() >= 70) {
				createitem = new int[] { 50642 }; // シャルナの変身スクロール（レベル80）
			}
			int createCount = 0;
			if (s.equalsIgnoreCase(":")) { // 11
				...
				createCount = 17;
			} else if (s.equalsIgnoreCase("A")) { // 18
				createCount = 18;
			} else if (s.equalsIgnoreCase("B")) { // 19
				createCount = 19;
			} else if (s.equalsIgnoreCase("C")) { // 20
				createCount = 20;
			} else {
				createCount = Integer.parseInt(s) + 1;
			}
			if (createitem != null) {
		...
		} else if (npcTemp.getNpcId() == 70035
				|| npcTemp.getNpcId() == 70041
				|| npcTemp.getNpcId() == 70042) { // ギランレース管理人
			// 　
			// セシル
			// 　
			// ポーリー
		...
		else if (npcTemp.getNpcId() == 91327) {
			if (s.equalsIgnoreCase("c")) {
		...
		else if (npcTemp.getNpcId() == 71179) {
		...
		else if (npcTemp.getNpcId() == 71178) {
		...
		else if (npcTemp.getNpcId() == 71200) {
			if (s.equalsIgnoreCase("a")) {
				pc.getQuest().setStep(L1Quest.QUEST_LEVEL45, 3);
		...
		else if (npcTemp.getNpcId() == 70739) {
			if (pc.isCrown()) {
				if (s.equalsIgnoreCase("e")) {
		...
		} else if (npcTemp.getNpcId() == 91308) { // 捨てられた肉体
			if (s.equalsIgnoreCase("a")) {
		...
		else if (npcTemp.getNpcId() == 80192) {
		...
		// ペット商人・引き換え
		else if (npcTemp.getNpcId() == 70077 // ロドニー
				|| npcTemp.getNpcId() == 91056) { // バンク
			SellOfPet(s, pc);
			htmlid = "";
		}
		// ヒレン
		else if (npcTemp.getNpcId() == 80238) {
			if (s.equalsIgnoreCase("a")) {
		...
		// ジェフ
		else if (npcTemp.getNpcId() == 80239) {
		...
		// 孤独な精霊
		else if (npcTemp.getNpcId() == 80242) {
			if (s.equalsIgnoreCase("a")) {
		...
		// 初心者案内人(新隠された渓谷)
		} else if (npcTemp.getNpcId() == 80171) {
			int level = pc.getLevel();
			if (pc.getLevel() < 2) {
		...
		// 修練場管理員
		} else if (npcTemp.getNpcId() == 80172) {
			if (s.equalsIgnoreCase("A")) {
				giveBeginnerItems(pc);
				htmlid = "";
			}
		// ホバン
		} else if (npcTemp.getNpcId() == 80179) {
		...
		// 旅人案内人
		} else if (npcTemp.getNpcId() == 80180) {
			int level = pc.getLevel();
			if (level < 46) {
		...
		// 鍛冶屋 クス
		} else if (npcTemp.getNpcId() == 70517) {
		...
		// キルトン
		} else if (npcTemp.getNpcId() == 80263) {
		...
		// メーリン
		} else if (npcTemp.getNpcId() == 80264) {
		...
		// 特産物管理人 モポ
		} else if (npcTemp.getNpcId() == 80266) {
		...
		// 強化ウィザード
		} else if ((npcTemp.getNpcId() >= 81352)
				&& (npcTemp.getNpcId() <= 81362) || npcTemp.getNpcId() == 71279) {
			if (s.equals("a") || s.equals("b") || s.equals("c")){
				...
				if (pc.getInventory().checkItem(50697)) { // 黒蛇のコイン
					L1SkillUse l1skilluse = new L1SkillUse();
					l1skilluse.handleCommands(pc, SPIRIT_OF_BLACK_SNAKE, pc.getId(),
							pc.getX(), pc.getY(), null, 0, L1SkillUse.TYPE_NPCBUFF);
					htmlid = "";
				}
			} else if (s.equals("0")) {
		...
		// ドラゴンポータル(隠された竜の地)
		} else if (npcTemp.getNpcId() == 91066) {
			int level = pc.getLevel();
		...
		// ネットカフェマップ
		} else if (npcTemp.getNpcId() == 51000) {
			// 正確なテレポート位置が不明
			if (s.equalsIgnoreCase("a")) {
		...
		// アイテム制作師 シュエルメ
		} else if (npcTemp.getNpcId() == 91061) {
		...
		// クレイ
		} else if (npcTemp.getNpcId() == 91062) {
			if (s.equalsIgnoreCase("a")) {
				L1BuffUtil.effectBlessOfDragonSlayer(pc, BLESS_OF_CRAY, 2400, 7681); // クレイの祝福
				htmlid = "grayknight2";
			}
		// サエル
		} else if (npcTemp.getNpcId() == 91500) {
			if (s.equalsIgnoreCase("a")) {
		...
		// グンター
		} else if (npcTemp.getNpcId() == 91600) {
			if (s.equalsIgnoreCase("a")) {
		...
		// コマ
		else if (npcTemp.getNpcId() == 80194) {
			htmlid = L1EnchantOfComaActions.getInstance().actions(objid, npcTemp, s, pc);
		}
		// ゾウのラヴァゴーレム
		else if (npcTemp.getNpcId() == 99020) {
			htmlid = L1GolemOfLavaActions.getInstance().actions(npcTemp, s, pc);
		}
		// ピアス
		else if (npcTemp.getNpcId() == 70908) {
			htmlid = L1PiersNpcActions.getInstance().actions(npcTemp, s, pc);
		}
		// スロット解放専門家^スナッパー
		else if (npcTemp.getNpcId() == 81375) {
			htmlid = L1OpenSlotNpcActions.getInstance().actions(npcTemp, s, pc);
		}
		// テレポーター・ブラッド / マービン / 象牙の塔の諜報員
		else if (npcTemp.getNpcId() == 70751 || npcTemp.getNpcId() == 81376
				|| npcTemp.getNpcId() == 71276) {
			htmlid = L1IceQueenCastleNpcActions.getInstance().actions(npcTemp, s, pc);
		}
		// 脱出したガーディアン
		else if (npcTemp.getNpcId() == 71277) {
			htmlid = L1HosterityWeaponNpcActions.getInstance().actions(objid, npcTemp, s, pc);
		}
		// 時の歪み（日ノ本）
		else if (npcTemp.getNpcId() == 71280 ||
				(46293 <= npcTemp.getNpcId() && npcTemp.getNpcId() <= 46299) ||
				(46524 <= npcTemp.getNpcId() && npcTemp.getNpcId() <= 46526)) {
			htmlid = L1HinomotoNpcActions.getInstance().actions(npcTemp, s, pc, obj);
		}
		// ジークフリード
		else if (npcTemp.getNpcId() == 71278) {
			htmlid = L1LastavardNpcActions.getInstance().actions(npcTemp, s, pc);
		}
		// ソウルクリスタル：BOSS関係
		else if (npcTemp.getNpcId() >= 71281 && npcTemp.getNpcId() <= 71283) {
			htmlid = L1BossKeyNpcActions.getInstance().actions(npcTemp, s, pc);
		}
		// ドルーガベール（ドラゴンキー販売）
		else if (npcTemp.getNpcId() == 91050) {
			htmlid = L1VeilActions.getInstance().actions(npcTemp, s, pc);
		} else {
			htmlid = L1EventNpcAction.getInstance().actions(npcTemp, s, pc);
		}

		// else System.out.println("C_NpcAction: " + s);
		if (htmlid != null && htmlid.equalsIgnoreCase("colos2")) {
			htmldata = makeUbInfoStrings(npcTemp.getNpcId());
		}
	...
	private String watchUb(L1PcInstance pc, int npcId) {
		int ubId = UltimateBattleConfigTable.getInstance().getUltimateBattleIdForNpc(npcId);
		UltimateBattleController ub = UltimateBattleTimeController.getInstance().getUBController(ubId);
		L1Location loc = ub.getConfig().getLocation();
	...
	private String enterUb(L1PcInstance pc, int npcId) {
		...
		if (ub.getPlayers().size() >= ub.getConfig().getMaxPlayer()) { // 定員オーバー
			return "colos4";
		}

		ub.addPlayer(pc); // メンバーに追加
		L1Location loc = ub.getConfig().getLocation().randomLocation(10, false);
		L1Teleport.teleport(pc, loc.getX(), loc.getY(), ub.getConfig().getMapId(), 5, true);
		return "";
	}
