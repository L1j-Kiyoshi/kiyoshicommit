...
public class S_NpcTalkReturn extends ServerBasePacket {
	...
	public S_NpcTalkReturn(L1NpcTalkData npc, int objid, int action,
		...
		if (getHtmlid(npc.getNpcID()) != "") {
			htmlid = getHtmlid(npc.getNpcID());
		} else if (action == 1) {
	...
	private String getHtmlid(int npcId) {
		String htmlid = "";
		if (npcId == 71255) { // テーベ オシリス祭壇のキーパー
			if (ThebesRaidController.getInstance().isEnterTime()) {
				htmlid = "tebegate1";
			} else {
				htmlid = "tebegate2";
			}
		} else if (npcId == 90521) {
			if (KukulcanRaidController.getInstance().isEnterTime()) {
				htmlid = "tikalgate1";
			} else {
				htmlid = "tikalgate2";
			}
