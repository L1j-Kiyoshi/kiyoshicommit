...
public class S_ShopSellList extends ServerBasePacket {
	...
	public S_ShopSellList(int objId) {
		...
		for (int i = 0; i < shopItems.size(); i++) {
			...
			int price;
			if (shopItem.isLayTax()) { // 課税アイテム
				price = calc.layTax((int)
		...
		int CurrencyItemId =  NpcTalkDataTable.getInstance().getTemplate(npcId).getCurrencyItemId();
		if (CurrencyItemId == 40308) {
			writeH(0x07); // 0x00:kaimo 0x01:pearl 0x07:adena
		} else {
			writeH(0x02); // 0x00:kaimo 0x01:pearl 0x07:adena
		}
	}
