...
public class InnKeyTable {
	...
	public static void deleteKey(L1ItemInstance item) {
		...
		PreparedStatement pstm = null;
		try {
			L1InstanceMap.getInstance().removeInstanceMap(item.getKeyId());
			con = L1DatabaseFactory.getInstance().getConnection();
	...
	public static boolean isUsedMap(int mapId) {
		Connection con = null;
		PreparedStatement pstm = null;
		ResultSet rs = null;
		...
		try {
			con = L1DatabaseFactory.getInstance().getConnection();
			pstm = con.prepareStatement("SELECT * FROM inn_keys WHERE id=?");
			pstm.setInt(1, mapId);
			rs = pstm.executeQuery();
			if (rs.next()) {
				isUsed = true;
