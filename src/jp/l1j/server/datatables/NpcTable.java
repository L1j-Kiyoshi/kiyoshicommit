...
public class NpcTable {
	...
	public static synchronized NpcTable getInstance() {
		if (_instance == null) {
	...
	private void loadNpcs(HashMap<Integer, L1Npc> npcs, HashMap<String, Constructor<?>> constructorCache) {
		...
		try {
			...
			while (rs.next()) {
				...
				npc.setResistEarth(rs.getInt("resist_earth"));
				npc.setResistFire(rs.getInt("resist_fire"));
				...
				npc.setPoisonInterval(rs.getInt("poison_interval"));
				npc.setPoisonDamage(rs.getInt("poison_damage"));
				...
				npc.setDeadTimer(rs.getInt("dead_timer"));
