...
public class CharBuffTable {
	...
	private static final int[] BUFF_SKILL_IDS = {
		ICE_LANCE, FOG_OF_SLEEPING, EARTH_BIND, THUNDER_GRAB,
		...
		STATUS_ELFBRAVE, STATUS_RIBRAVE, CUBE_IGNITION_ALLY, CUBE_QUAKE_ALLY, CUBE_SHOCK_ALLY,
		STATUS_THIRD_SPEED, STATUS_FLORA_POTION_STR, STATUS_FLORA_POTION_DEX, STATUS_FLORA_POTION_AC,
		STATUS_CURSE_BARLOG, STATUS_CURSE_YAHEE,
		STATUS_WEAKNESS_EXPOSURE_LV1, STATUS_WEAKNESS_EXPOSURE_LV2, STATUS_WEAKNESS_EXPOSURE_LV3,
	...
	private static void restore(int objId, int skillId, int time, int polyId, int attrKind) {
	...
	private static boolean saveByExecutor(int skillId, L1PcInstance pc) {
		...
		restore(buff.getCharcterId(), buff.getSkillId(), buff.getRemainingTime(),
	...
	public static void save(L1PcInstance pc) {
		for (int skillId : buffSkillIds()) {
			...
			if (0 < timeSec) {
				...
				restore(pc.getId(), skillId, timeSec, polyId, 0);
			}
		}
	}
