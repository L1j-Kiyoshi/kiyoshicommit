...
public class SprTable {
	...
	// <GfxId, SprSpeed>
	private static HashMap<Integer, SprSpeed> _allSprActions
											= new HashMap<Integer, SprSpeed>();

	private static HashMap<Integer, Spr> _sprActions = new HashMap<Integer, Spr>();

	private static class Spr {
		private final HashMap<Integer, Integer> allSpeed = new HashMap<Integer, Integer>();
	...
	class SprSpeed {
		private HashMap<Integer, Integer> _sprSpeed = new HashMap<Integer, Integer>();

		public HashMap<Integer, Integer> getSprSpeed() {
			return _sprSpeed;
		}
	}
	...
	public void loadSprActions(HashMap<Integer, Spr> sprActions) {
		HashMap<Integer, Integer> actionTime = new HashMap<Integer, Integer>();
		Connection con = null;
		...
		try {
			...
			while (rs.next()) {
				...
				if (_allSprActions.get(key) == null) {
					_allSprActions.put(key, new SprSpeed());
				}
				_allSprActions.get(key).getSprSpeed().put(actid, speed);
			}
	...
	/**
	 * 引数sprIdに対応するHashMapを返す。
	 * @param sprId
	 * @return
	 * 		HashMap(ActionId, Speed)
	 */
	public HashMap<Integer, Integer> getActionList(int sprId) {
		return _allSprActions.get(sprId).getSprSpeed();
	}
	...
	/**
	 * 指定されたsprの攻撃速度を返す。もしsprに指定されたactIdのデータが 設定されていない場合は、1.attackのデータを返す。
	 *
	 * @param sprid -
	 *            調べるsprのID
	 * @param actid -
	 ...
	 * @return 指定されたsprの攻撃速度(ms)
	 */
	public int getSpeed(int sprid, int actid) {
		if (_sprActions.containsKey(sprid)) {
			if (_sprActions.get(sprid).allSpeed.get(actid) == null) {
				return _sprActions.get(sprid).attackSpeed.get(ACTION_Attack);
			} else {
				return _sprActions.get(sprid).allSpeed.get(actid);
			}
		} else {
			return _sprActions.get(sprid).attackSpeed.get(ACTION_Attack);
		}
