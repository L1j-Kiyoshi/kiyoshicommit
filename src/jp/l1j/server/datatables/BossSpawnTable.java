...
public class BossSpawnTable {
	...
	private static Map<Integer, L1BossSpawn> _spawntable = new HashMap<Integer, L1BossSpawn>();
	...
	public static void fillSpawnTable() {
		...
		try {
			...
			pstm = con.prepareStatement("SELECT * FROM spawn_boss_mobs WHERE event_id = 0");
			...
			while (rs.next()) {
				...
				if (template1 == null) {
					...
					spawnDat.setEventId(rs.getInt("event_id"));
					...
					String randMapIds = rs.getString("random_map_id");
					if (randMapIds != null) {
						StringTokenizer st = new StringTokenizer(randMapIds, ",");
						while (st.hasMoreTokens()) {
							try {
								spawnDat.addRandomMapId(Short.valueOf(st.nextToken()));
							} catch (Exception e) {
								_log.log(Level.SEVERE, e.getLocalizedMessage(), e);
								_log.fine("mob data for id:" + npcTemplateId + " missing in spawn_boss_mobs table");
							}
						}
					}
	...
	public static void fillSpawnTableByEvent(int eventId) {
		int spawnCount = 0;
		java.sql.Connection con = null;
		PreparedStatement pstm = null;
		ResultSet rs = null;
		try {
			con = L1DatabaseFactory.getInstance().getConnection();
			pstm = con.prepareStatement("SELECT * FROM spawn_boss_mobs WHERE event_id = ?");
	...
	public static L1BossSpawn getTemplate(int i) {
		return _spawntable.get(i);
	}

	public static void removeSpawn(int spawnId) {
