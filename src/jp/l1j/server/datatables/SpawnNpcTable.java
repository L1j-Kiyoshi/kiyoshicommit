...
//		fillNpcSpawnTable();
	...
	public void fillNpcSpawnTable() {
		...
		try {
			...
			pstm = con.prepareStatement("SELECT * FROM spawn_npcs WHERE event_id = 0");
	...
	public void fillNpcSpawnTableByEvent(int eventId) {
		int spawnCount = 0;
		Connection con = null;
		PreparedStatement pstm = null;
		ResultSet rs = null;
		try {
			con = L1DatabaseFactory.getInstance().getConnection();
			pstm = con.prepareStatement("SELECT * FROM spawn_npcs WHERE event_id=?");
			...
			while (rs.next()) {
				...
				L1Spawn l1spawn;
				if (l1npc == null) {
					_log.warning("event npc data for id:" + npcId + " missing in npcs table");
					l1spawn = null;
				} else {
					if (rs.getInt("count") == 0) {
						continue;
					...
					l1spawn = new L1Spawn(l1npc);
					l1spawn.setId(rs.getInt("id"));
					l1spawn.setEventId(rs.getInt("event_id"));
					l1spawn.setAmount(rs.getInt("count"));
					l1spawn.setLocX(rs.getInt("loc_x"));
					l1spawn.setLocY(rs.getInt("loc_y"));
					l1spawn.setRandomx(rs.getInt("random_x"));
		...
		_log.fine("loaded event npc: " + _spawntable.size() + " records");
	}
	...
	public void storeSpawn(L1PcInstance pc, L1Npc npc) {
		...
		try {
			...
			con = L1DatabaseFactory.getInstance().getConnection();
			pstm = con.prepareStatement("INSERT INTO spawn_npcs SET npc_id=?,note=?,event_id=?,count=?,loc_x=?,loc_y=?,heading=?,map_id=?");
