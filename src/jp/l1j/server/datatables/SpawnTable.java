...
//		fillSpawnTable();
	...
	public void fillSpawnTable() {
		...
		try {
			...
			pstm = con.prepareStatement("SELECT * FROM spawn_mobs WHERE event_id = 0");
	...
	public void fillSpawnTableByEvent(int eventId) {
		int spawnCount = 0;
		java.sql.Connection con = null;
		PreparedStatement pstm = null;
		ResultSet rs = null;
		try {
			PerformanceTimer timer = new PerformanceTimer();
			con = L1DatabaseFactory.getInstance().getConnection();
			pstm = con.prepareStatement("SELECT * FROM spawn_mobs WHERE event_id=?");
			...
			_log.fine("loaded event mob: " + _spawntable.size() + " records");
			System.out.println("loading event mobs...OK! " + timer.elapsedTimeMillis() + " ms");
		} catch (SQLException e) {
			_log.log(Level.SEVERE, e.getLocalizedMessage(), e);
		} finally {
			SqlUtil.close(rs);
		...
		_log.fine("loaded event mob: " + spawnCount + " records");
	}
	...
	public static void storeSpawn(L1PcInstance pc, L1Npc npc) {
		...
		try {
			...
			con = L1DatabaseFactory.getInstance().getConnection();
			pstm = con.prepareStatement(String.format("INSERT INTO spawn_mobs SET %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s",
				"npc_id=?", "note=?", "event_id=?", "count=?", "loc_x=?", "loc_y=?",
