...
import jp.l1j.server.controller.timer.MapTimerResetController;
...
public class GameServer extends Thread {
	...
	private static boolean isAutoRestart;
	...
	public void initialize() throws Exception {
		isAutoRestart = Config.AUTO_RESTART;
		printStartupMessage();
		...
		UltimateBattleTimeController ubtc = UltimateBattleTimeController.getInstance();
		GeneralThreadPool.getInstance().execute(ubtc);
		...
		// 時の歪みタイムコントローラー
		DistortionOfTimeController distortionController = DistortionOfTimeController.getInstance();
		GeneralThreadPool.getInstance().execute(distortionController);

		// マップタイマーモニタリングコントローラー
		MapTimerController mapTimerController = MapTimerController.getInstance();
		GeneralThreadPool.getInstance().execute(mapTimerController);

		// マップタイマーリセットコントローラー
		MapTimerResetController mtrc = MapTimerResetController.getInstance();
		GeneralThreadPool.getInstance().execute(mtrc);

		// イベントコントローラー
		EventController ec = EventController.getInstance();
		GeneralThreadPool.getInstance().execute(ec);
		...
		SpawnTable.getInstance().fillSpawnTable();
		SpawnNpcTable.getInstance().fillNpcSpawnTable();
	...
	public void shutdown() {
		...
		if (isAutoRestart) {
			try {
				String[] filename = new String[] {"cmd.exe", "/C", "START", "ServerRestart.bat"};
