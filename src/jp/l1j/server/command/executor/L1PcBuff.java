	...
	public void execute(L1PcInstance pc, String cmdName, String arg) {
		...
		try {
			...
			int[] pcBuffSkill = { // L1SkillId から必要なBuffを入力
				...
				VENOM_RESIST, UNCANNY_DODGE, DRESS_EVASION, GLOWING_WEAPON,
				BRAVE_MENTAL, RESIST_MAGIC, CLEAR_MIND, RESIST_ELEMENTAL,
			...
			for (L1PcInstance tg : players) {
				...
				for (int element : pcBuffSkill) {
					...
					new L1SkillUse().handleCommands(tg, element, tg.getId(),
							tg.getX(), tg.getY(), null, (int) skill.getBuffDuration(), L1SkillUse.TYPE_GMBUFF);
				}
