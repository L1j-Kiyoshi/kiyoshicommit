...
public class L1PcInventory extends L1Inventory {
	...
	public void loadItems() {
		...
		for (L1ItemInstance item : L1InventoryItem.instantiate(inventoryItems)) {
			...
			if (item.getItemId() == 50678) { // 訓練場のキーの場合関連付けが必要
				InnKeyTable.hasKey(item);
			}
			if (item.getKeyId() >= 1400 && item.getKeyId() <= 1498) {
				if (item.getKeyId() > 1400) {
					L1InstanceMap.getInstance().addInstanceMap(1400, item.getKeyId());
				}
				L1NpcInstance npc = NpcTable.getInstance().newNpcInstance(71283); // 訓練副官^セシリア
				npc.setId(IdFactory.getInstance().nextId());
				npc.setMap((short) item.getKeyId());
				npc.getLocation().set(32902, 32818, item.getKeyId());
	...
	/**
	 * idのアイテムをcount数装備しているか
	 * @param id
	 * @param count
	 * @return
	 * 		idのアイテムをcount数装備していればtrue
	 */
	public boolean checkEquipped(int id, int count) {
		int max = count;
		for (Object itemObject : _items) {
			L1ItemInstance item = (L1ItemInstance) itemObject;
			if (item.getItem().getItemId() == id && item.isEquipped()) {
