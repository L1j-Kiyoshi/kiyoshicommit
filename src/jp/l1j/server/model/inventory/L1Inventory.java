...
public class L1Inventory extends L1Object {
	...
	// 強制的にアイテムをデータベースから削除。
	public void deleteItem(int objId, L1ItemInstance item) {
		L1InventoryItem.delete(objId);
		onRemoveItem(item);
		_items.remove(item);
	}
	...
	// アイテムＩＤから検索
	public L1ItemInstance getItemByItemId(int itemId) {
		for (Object itemObject : _items) {
			L1ItemInstance item = (L1ItemInstance) itemObject;
	...
	public boolean checkEnchantItem(int id, int enchant, int count) {
		...
		for (L1ItemInstance item : _items) {
			...
			if (item.isSealed()) { // 封印しているものは該当しない
				continue;
			}
