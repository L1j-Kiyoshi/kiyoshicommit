...
public class L1TeleporterInstance extends L1NpcInstance {
	...
	public void onTalkAction(L1PcInstance pc) {
		...
		String htmlid = null;
		if (npcid == 50128) { // アンタラスのレア
			int x = 32795 + _random.nextInt(3);
			if (!DragonPortalController.getInstance().getDragonRaid(pc.getMapId()).isDragonAwaked()) {
				L1Teleport.teleport(pc, x, 32662, pc.getMapId(), pc.getHeading(), true);
				DragonPortalController.getInstance().startDragonRaid(pc.getMapId());
				DragonPortalController.getInstance().getDragonRaid(pc.getMapId()).addDragonSlayers(pc);
			} else {
				pc.sendPackets(new S_ServerMessage(1537)); // ドラゴンが目覚めたため、今は入場できません。
			}
		} else if (npcid == 50129) { // パプリオンのレア
			int y = 32841 + _random.nextInt(3);
			if (!DragonPortalController.getInstance().getDragonRaid(pc.getMapId()).isDragonAwaked()) {
				L1Teleport.teleport(pc, 32992, y, pc.getMapId(), pc.getHeading(), true);
				DragonPortalController.getInstance().startDragonRaid(pc.getMapId());
				DragonPortalController.getInstance().getDragonRaid(pc.getMapId()).addDragonSlayers(pc);
			} else {
				pc.sendPackets(new S_ServerMessage(1537)); // ドラゴンが目覚めたため、今は入場できません。
			}
		} else if (npcid == 50130) { // リンドビオルのレア
			int x = 32831 + _random.nextInt(3);
			if (!DragonPortalController.getInstance().getDragonRaid(pc.getMapId()).isDragonAwaked()) {
				L1Teleport.teleport(pc, x, 32893, pc.getMapId(), pc.getHeading(), true);
				DragonPortalController.getInstance().startDragonRaid(pc.getMapId());
				DragonPortalController.getInstance().getDragonRaid(pc.getMapId()).addDragonSlayers(pc);
			} else {
				pc.sendPackets(new S_ServerMessage(1537)); // ドラゴンが目覚めたため、今は入場できません。
