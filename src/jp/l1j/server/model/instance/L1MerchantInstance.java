...
public class L1MerchantInstance extends L1NpcInstance {
	...
	public void onTalkAction(L1PcInstance pc) {
		...
		if (talking != null) {
			...
			} else if (npcid == 81376) { // マービン(100日クエスト)
				if (pc.isMarbinQuest()) { // マービンの袋を1度もらっている
					htmlid = "marbinquest3";
				} else {
					htmlid = "marbinquest1";
				}
			}
/*			else if (npcid == 91050) { // ドルーガベール
				if (Config.SELL_DRAGON_KEY) { // 本鯖通り
					htmlid = "veil1";
				} else {
					htmlid = "lind_veil";
				}
...
*/
			else if (npcid == 46524) { // 左之助(割れた器クエスト:52～59)
				if (pc.getLevel() < 52) { // 52未満
					htmlid = "jp_sanojo2";
				} else if (pc.getLevel() >= 60) { // 60以上
					htmlid = "jp_sanojo3";
				} else {
					htmlid = "jp_sanojo1";
				}
