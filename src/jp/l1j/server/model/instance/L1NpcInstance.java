...
import jp.l1j.server.model.DeadTimer;
...
public class L1NpcInstance extends L1Character {
	...
	private boolean AIProcess() {
		...
		if (getNpcTemplate().getDeadTimer() > 0 && getDeadTimer() == null) {
			setDeadTimer(new DeadTimer(null, this, getNpcTemplate().getDeadTimer()));
			getDeadTimer().start();
		}
	...
	private boolean useSkill(L1Character target, boolean isPhysicalAttack) {
		List<L1AvailableSkill> skills = _mobSkill.availableSkills(target, true);
		if (skills.isEmpty()) { // トリガの条件に合うスキルがない
			return false;
		}
		if (isPhysicalAttack && _random.nextInt(1000) >= 500) { // 物理攻撃が可能な位置では、一定確立で物理攻撃
			return false;
		}
	...
	public void onTarget() {
		...
		if (getAtkSpeed() == 0) { // 逃げるキャラ
			if (getPassiSpeed() > 0 && getSkillEffectTimeSec(THUNDER_GRAB) <= 0) { // 移動できるキャラ
			...
			if (isAttackPosition(target.getX(), target.getY(), getNpcTemplate().getRanged())) { // 攻撃可能位置
				if (useSkill(target, true)) {
				...
				if (useSkill(target, false)) { // ランダムにスキル使用
					setSleepTime(calcSleepTime(_mobSkill.getSleepTime(), MAGIC_SPEED));
					return;
				}

				if (getPassiSpeed() > 0 && getSkillEffectTimeSec(THUNDER_GRAB) <= 0) {
					// 移動できるキャラ
	...
	public void attackTarget(L1Character target) {
		...
		if (attack.calcHit()) {
			L1CounterBarrier cb = new L1CounterBarrier(target, this);
			if (cb.isCounterBarrier()) { // targetがCB発動条件を満たしている
	...
	public boolean noTarget() {
		if (_master != null && _master.getMapId() == getMapId()
			...
			// 移動できるキャラはランダムに動いておく
			if (_master == null && getPassiSpeed() > 0 && !isRest() && getSkillEffectTimeSec(THUNDER_GRAB) <= 0) {
	...
	public void setting_template(L1Npc template) {
		...
		addEarth(template.getResistEarth());
		addFire(template.getResistFire());
		addWater(template.getResistWater());
		addWind(template.getResistWind());
		addLight(template.getResistLight());
	...
	public void transformByCancellation(int transformId) {
