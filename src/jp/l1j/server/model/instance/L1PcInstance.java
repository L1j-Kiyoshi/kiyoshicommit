...
import jp.l1j.server.model.LindviorStormEffect;
...
public class L1PcInstance extends L1Character {
	...
	// TODO 定期表示エフェクト開始
	public void startRegularlyEffect(int skillId) {
		if (skillId == DIVINE_PROTECTION_OF_FAFURION) { // パプリオンハイドロエフェクト
			_regularlyEffectList.put(skillId, new FafurionHydroEffect(this));
		} else if (skillId == DIVINE_PROTECTION_OF_LINDVIOR_PLATE) { // リンドビオルストームエフェクト
			_regularlyEffectList.put(skillId, new LindviorStormEffect(this));
		} else if (skillId == DIVINE_PROTECTION_OF_LINDVIOR_LEATHER) { // リンドビオルストームエフェクト
			_regularlyEffectList.put(skillId, new LindviorStormEffect(this));
		} else if (skillId == DIVINE_PROTECTION_OF_LINDVIOR_SCALE) { // リンドビオルストームエフェクト
			_regularlyEffectList.put(skillId, new LindviorStormEffect(this));
		} else if (skillId == DIVINE_PROTECTION_OF_LINDVIOR_ROBE) { // リンドビオルストームエフェクト
			_regularlyEffectList.put(skillId, new LindviorStormEffect(this));
		} else if (skillId == DIVINE_PROTECTION_OF_HALLOWEEN) { // ハロウィンエフェクト
			_regularlyEffectList.put(skillId, new HalloweenEffect(this));
		}
		_regularlyEffectActiveList.put(skillId, true);
		_timer.scheduleAtFixedRate(_regularlyEffectList.get(skillId), 60000, 60000);
	}

	// TODO 定期表示エフェクト終了
	public void stopRegularlyEffect(int skillId) {
		if (_regularlyEffectActiveList.get(skillId) != null &&
				_regularlyEffectActiveList.get(skillId)) {
			_regularlyEffectActiveList.put(skillId, false);
			_regularlyEffectList.get(skillId).cancel();
		}
	}

	// パプリオンの加護発動
	public void commitFafurionHydroArmor() {

		if ((_random.nextInt(1000) + 1) <= 50) { // 5%の確率で回復
			sendPackets(new S_SkillSound(getId(), 2187));
			broadcastPacket(new S_SkillSound(getId(), 2187));
			int healHp = _random.nextInt(20) + 70;
			if (hasSkillEffect(WATER_LIFE)) {
				removeSkillEffect(WATER_LIFE);
				healHp *= 2;
			}
			if (hasSkillEffect(POLLUTE_WATER)) {
				healHp /= 2;
			}
			if (hasSkillEffect(DEATH_HEAL)) {
				healHp *= -1;
			}
			int newHp = getCurrentHp() + healHp;
			if (newHp > getMaxHp()) {
				newHp = getMaxHp();
			}
			setCurrentHp(newHp);
	...
	// ハロウィンの加護発動
	public void commitDragonicHalloweenArmor() {

		if ((_random.nextInt(1000) + 1) <= 50) { // 5%の確率で回復
			sendPackets(new S_SkillSound(getId(), 2183));
			broadcastPacket(new S_SkillSound(getId(), 2183));
			int healHp = 31;
			if (hasSkillEffect(WATER_LIFE)) {
				removeSkillEffect(WATER_LIFE);
				healHp *= 2;
	...
	// 魔力のリンドビオルの加護発動
	public void commitLindoviorArmor(int dmg) {
		L1ItemInstance armor = getInventory().getItemEquipped(2, 3);
		if (armor == null) {
			return;
		}
		int chance = (armor.getEnchantLevel() + 1) * 10;
		if ((_random.nextInt(1000) + 1) <= chance) { // 強化値+1の確率で回復?
			sendPackets(new S_SkillSound(getId(), 2188));
			broadcastPacket(new S_SkillSound(getId(), 2188));
			int healMp = _random.nextInt(9);
			if (hasSkillEffect(DIVINE_PROTECTION_OF_LINDVIOR_PLATE) && dmg >= 30) {
				healMp += 6; // ローブについて発動条件は被ダメージ11。
			}
			if (hasSkillEffect(DIVINE_PROTECTION_OF_LINDVIOR_SCALE) && dmg >= 25) {
				healMp += 8;
			}
			if (hasSkillEffect(DIVINE_PROTECTION_OF_LINDVIOR_LEATHER) && dmg >= 20) {
				healMp += 15;
			}
			if (hasSkillEffect(DIVINE_PROTECTION_OF_LINDVIOR_ROBE) && dmg >= 11) {
				healMp += 21;
			}

			int newMp = getCurrentMp() + healMp;
			if (newMp > getMaxMp()) {
				newMp = getMaxMp();
			}
			setCurrentMp(newMp);
	...
	private int _killCount; // ● KILLカウント
	...
	private int _deathCount; // ● Deathカウント
	...
	public void onAction(L1PcInstance attacker, int skillId) {
		...
		if (getCurrentHp() > 0 && !isDead()) {
			...
			if (attack.calcHit()) {
				L1CounterBarrier cb = new L1CounterBarrier(this, attacker);
				if (cb.isCounterBarrier()) { // カウンターバリア発動条件を満たす。
					cb.commitCounterBarrier();
				} else { // カウンターバリア、リンドビオルの加護未発動
	...
	private class Death implements Runnable {
		...
		public void run() {
			...
			// TODO PK情報 start
			if (Config.PK_LOG) { // pklog on/off判定
				if (lastAttacker instanceof L1PcInstance) {
					L1World world = L1World.getInstance();
					String areaname = "";
					String Clanname1 = "";
					...
					StringBuffer killer = new StringBuffer("【Kill】 ");
					killer.append(lastAttacker.getName() + "[" + Clanname1 + "]"); // LastAttacker
					killer.append(" Kill(" + player.getKillCount() + ") Death(" + player.getDeathCount() + ")");
					StringBuffer deather = new StringBuffer("【Death】 ");
					deather.append(getName() + "[" + Clanname2 + "]");
					deather.append(" Kill(" + getKillCount() + ") Death(" + getDeathCount() + ")");
					StringBuffer logs = new StringBuffer(player.getName() + "が");
					logs.append(getName() + "を殺しました。");
					world.broadcastPacketToAll(new S_SystemMessage(I18N_PK_TEXT_COLOR + logs.toString()));
					world.broadcastPacketToAll(new S_SystemMessage(I18N_PK_TEXT_COLOR + killer.toString()));
					world.broadcastPacketToAll(new S_SystemMessage(I18N_PK_TEXT_COLOR + deather.toString()));
					world.broadcastPacketToAll(new S_SystemMessage(I18N_PK_TEXT_COLOR + "Location:" + areaname));
				}
			}
			// TODO PK情報 end
	...
	private HashMap<Integer, TimerTask> _regularlyEffectList = new HashMap<Integer, TimerTask>();
	private HashMap<Integer, Boolean> _regularlyEffectActiveList = new HashMap<Integer, Boolean>();
	...
	public double getMaxWeight() {
		...
		if (0 < Config.RATE_MAX_WEIGHT_LIMIT) { // 最大重量制限
			if (maxWeight > Config.RATE_MAX_WEIGHT_LIMIT) {
				maxWeight = Config.RATE_MAX_WEIGHT_LIMIT;
			}
		}
	...
	public boolean isFastMovable() {
		return (hasSkillEffect(HOLY_WALK) || hasSkillEffect(MOVING_ACCELERATION)
				|| hasSkillEffect(WIND_WALK) || hasSkillEffect(STATUS_BRAVE2));
	}
	...
	public boolean isBrave() {
		return hasSkillEffect(STATUS_BRAVE) || hasSkillEffect(BLOODLUST);
	}
	...
	private boolean _isPolyItemEquipped = false; // 変身アイテムを装備しているか。

	public boolean isPolyItemEquipped() {
		return _isPolyItemEquipped;
	}

	public void setPolyItemEquipped(boolean b) {
		_isPolyItemEquipped = b;
	}
	...
	public void resetBaseDmgup() {
		...
		if (isKnight() || isDarkelf() ||  // ナイト、ダークエルフ
				isDragonKnight() || isIllusionist()) { // ドラゴンナイト、イリュージョニスト
	...
	private int _teairorChance = 0; // 反逆者のシールドリダクション確率

	public int getTeairorChance() {
		return _teairorChance;
	}
	...
	public int getRedEarringChance() {
		return _redEarringChance;
	}
	...
	// <AreaId, Time>
	private Map<Integer, Integer> _mapTimer = null; // MAP時間制限

	public Map<Integer, Integer> getMapTimer() {
		return _mapTimer;
	}

	public void setMapTimer(Map<Integer, Integer> map) {
		_mapTimer = map;
	}

	/**
	 * キャラクターの設定を保存する項目<br><br>
	 * 1bit: PotLog<br>
	 * 2bit: DropLog(自分のドロップログが見えるか)<br>
	 * 3bit: PartyDropLog(パーティ時自分以外のドロップログを流すか)<br>
	 * 5bit: LawfulGet (モンスター討伐時のアライメントの増減を有効にするか)<br>
	 *<br>
	 * 1: true or on<br>
	 * 0: false or off<br>
	 */
	private int _config = 65535;
	...
	public void saveConfig() {
		CharacterTable.getInstance().updateCharConfig(this);
	}

	public boolean isPotLog() {
		return (_config & 0x01) == 0x01;
	}

	public void setPotLog(boolean flg) {
		if (flg) {
			_config = (_config | 0x01);
		} else {
			_config = (_config & 0xFFFE);
		}
	}

	public boolean isDropLog() {
		return (_config & 0x02) == 0x02;
	}

	public void setDropLog(boolean flg) {
		if (flg) {
			_config = (_config | 0x02);
		} else {
			_config = (_config & 0xFFFD);
		}
	}

	public boolean isPartyDropLog() {
		return (_config & 0x04) == 0x04;
	}

	public void setPartyDropLog(boolean flg) {
		if (flg) {
			_config = (_config | 0x04);
		} else {
			_config = (_config & 0xFFFB);
		}
	}

	public boolean isAlignmentGet() {
		return (_config & 0x08) == 0x08;
	}

	public void setAlignmentGet(boolean flg) {
		if (flg) {
			_config = (_config | 0x08);
		} else {
			_config = (_config & 0xFFF7);
		}
	}

	public boolean isRingSlotOpen76() {
		return (_config & 0x0100) == 0x0100;
	}

	public void setRingSlotOpen76(boolean flg) {
		if (flg) {
			_config = (_config | 0x0100);
		} else {
			_config = (_config & 0xFEFF);
		}
	}

	public boolean isRingSlotOpen81() {
		return (_config & 0x0200) == 0x0200;
	}

	public void setRingSlotOpen81(boolean flg) {
		if (flg) {
			_config = (_config | 0x0200);
		} else {
			_config = (_config & 0xFDFF);
		}
	}

	public boolean isMarbinQuest() {
		return (_config & 0x1000) == 0x1000;
	}

	public void setMarbinQuest(boolean flg) {
		if (flg) {
			_config = (_config | 0x1000);
		} else {
			_config = (_config & 0xEFFF);
		}
	}
}