...
public class L1BravePotion {
	...
	public boolean use(L1PcInstance pc, L1ItemInstance item) {
		if (pc.hasSkillEffect(DECAY_POTION)) { // ディケイ ポーションの状態
		...
		if (getItemId() == 40068 || getItemId() == 140068 || getItemId() == 41479) { // エルヴンワッフル・村人集中POT
			...
			if (pc.hasSkillEffect(DANCING_BLAZE)) { // ダンシングブレイズとは重複しない
				pc.killSkillEffectTimer(DANCING_BLAZE);
				pc.sendPackets(new S_SkillBrave(pc.getId(), 0, 0));
				pc.broadcastPacket(new S_SkillBrave(pc.getId(), 0, 0));
