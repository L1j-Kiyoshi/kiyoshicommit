...
public class L1TreasureBox {
	...
	public boolean open(L1PcInstance pc, L1ItemInstance box) {
		...
		if (getType().equals(TYPE.SPECIFIC)) {
			// 出るアイテムが決まっているもの
			for (Item each : getItems()) {
				if (isLastItem()) { // チャージアイテムで、最後にのみ出現するアイテムがあるもの。
					if (box.getChargeCount() > 1) { // チャージカウントがあるアイテム
						if (each.isLast()) { // 最終チャージのみ出現するアイテムはスルー。
							continue;
						}
					} else if (box.getChargeCount() == 1) { // チャージカウントがあるアイテムの最終
						if (!each.isLast()) { // 最終チャージには出現しないアイテムはスルー。
							continue;
						}
					}
				}
				...
				if (item != null) {
					if (each.getEnchant() > 0) {
						item.setEnchantLevel(each.getEnchant());
					}
					if (each.getCount() > 0) {
						if (!item.isStackable()) {
							for (int i = each.getCount(); i > 0; i--) {
								storeItem(pc, item);
								item = ItemTable.getInstance().createItem(each.getItemId());
								if (each.getEnchant() > 0) {
									item.setEnchantLevel(each.getEnchant());
