...
public class L1Tifsome extends L1UniqueSkillUtils implements Runnable {

	private L1Character _user;
	private L1Character _target;

	public L1Tifsome(L1Character user, L1Character target) {
		_user = user;
		_target = target;
	}
	...
	public void run() {
		try {
			// オーブモーク！ティフサム
			int sleepTime = 0;
			_user.broadcastPacket(new S_NpcChatPacket((L1NpcInstance) _user, "$7915", 0));
