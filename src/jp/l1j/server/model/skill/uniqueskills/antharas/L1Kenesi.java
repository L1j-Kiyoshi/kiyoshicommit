...
public class L1Kenesi extends L1UniqueSkillUtils implements Runnable {

	private L1Character _user;
	private L1Character _target;

	public L1Kenesi(L1Character user, L1Character target) {
		_user = user;
		_target = target;
	}
	...
	public void run() {
		try {
			...
			// 右爪
			L1PhysicalAttack rightClaw = PhysicalSkillsTable.getInstance().findBySkillId(999);
			...
			// 毒ブレス
			new L1SkillUse().handleCommands(null, 1036, _target.getId(), _target
					.getX(), _target.getX(), null, 0, L1SkillUse.TYPE_NORMAL, _user);
			...
			// 左爪
			L1PhysicalAttack leftClaw = PhysicalSkillsTable.getInstance().findBySkillId(997);
