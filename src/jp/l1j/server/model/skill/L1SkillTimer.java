...
class L1SkillStop {
	...
	public static void stopSkill(L1Character cha, int skillId) {
		...
		if (skillId == GLOWING_WEAPON) { // グローウィング ウェポン
			cha.addHitup(-5);
			cha.addBowHitup(-5);
			cha.addDmgup(-5);
			if (cha instanceof L1PcInstance) {
		...
		} else if (skillId == SHINING_SHIELD) { // シャイニング オーラ
			cha.addAc(8);
			if (cha instanceof L1PcInstance) {
				L1PcInstance pc = (L1PcInstance) cha;
		...
		} else if (skillId == DANCING_BLAZE) { // ダンシングブレイズ
			cha.setBraveSpeed(0);
			if (cha instanceof L1PcInstance) {
				L1PcInstance pc = (L1PcInstance) cha;
		...
		} else if (skillId == HOLY_WALK || skillId == MOVING_ACCELERATION // ホーリーウォーク、ムービングアクセレーション
				|| skillId == WIND_WALK || skillId == BLOODLUST) { //  ブラッドラスト、ウィンドウォーク
			cha.setBraveSpeed(0);
			if (cha instanceof L1PcInstance) {
				L1PcInstance pc = (L1PcInstance) cha;
				pc.sendPackets(new S_SkillBrave(pc.getId(), 0, 0));
		...
		} else if (skillId == SPIRIT_OF_BLACK_SNAKE) { // 黒蛇の気
			if (cha instanceof L1PcInstance) {
				L1PcInstance pc = (L1PcInstance) cha;
				pc.addAc(2);
				pc.addMaxHp(-20);
		...
		// ****** 状態変化が解けた場合
			...
			cha.setMoveSpeed(0);
		} else if (skillId == CUBE_IGNITION_ALLY) { // キューブ[イグニション]：味方
			cha.addFire(-30);
			if (cha instanceof L1PcInstance) {
				L1PcInstance pc = (L1PcInstance) cha;
				pc.sendPackets(new S_OwnCharAttrDef(pc));
			}
		} else if (skillId == CUBE_QUAKE_ALLY) { // キューブ[クエイク]：味方
			cha.addEarth(-30);
			if (cha instanceof L1PcInstance) {
				L1PcInstance pc = (L1PcInstance) cha;
				pc.sendPackets(new S_OwnCharAttrDef(pc));
			}
		} else if (skillId == CUBE_SHOCK_ALLY) { // キューブ[ショック]：味方
			cha.addWind(-30);
			if (cha instanceof L1PcInstance) {
				L1PcInstance pc = (L1PcInstance) cha;
				pc.sendPackets(new S_OwnCharAttrDef(pc));
		...
		} else if (skillId == STATUS_FLORA_POTION_AC) { // フローラ系ポーション：AC
			cha.addAc(5);
			if (cha instanceof L1PcInstance) {
				L1PcInstance pc = (L1PcInstance) cha;
				pc.sendPackets(new S_SkillIconShield(5, 0));
		...
		// NPCしか使用しないスキル
		else if (skillId == MUMMY_SKIN) { // マミーロードスキン
				cha.addAc(50);
		}
