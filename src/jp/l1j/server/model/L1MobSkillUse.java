...
public class L1MobSkillUse {
	...
	private boolean summon(L1MobSkill skill, L1Character target) {
		...
		// 魔法を使う動作のエフェクト
		if (skill.getActId() > 0) {
			int sleepTime = SprTable.getInstance().getActionList(_attacker.getGfxId()).get(skill.getActId());
	 ...
	 */
	private boolean poly(L1MobSkill skill) {
		...
		for (L1PcInstance pc : L1World.getInstance()
			...
			L1PolyMorph.doPoly(pc, skill.getPolyId(), 1800, L1PolyMorph.MORPH_BY_NPC);
	...
	private boolean magicAttack(L1MobSkill mobSkill, L1Character target) {
		...
		if (canUseSkill == true) {
			...
			skillUse.handleCommands(null, skillId, target.getId(), target
					.getX(), target.getX(), null, 0, L1SkillUse.TYPE_NORMAL, _attacker);
			// 使用スキルによるsleepTimeの設定
			L1Skill skill = SkillTable.getInstance().findBySkillId(skillId);
			if (skill != null && skill.getRemoveDelay() != 0) {
				_sleepTime = skill.getRemoveDelay();
			} else if (skill.getActionId() > 0) {
				try {
					_sleepTime = SprTable.getInstance().getActionList(_attacker.getGfxId()).get(skill.getActionId());
				} catch (NullPointerException npe) {
					StringBuffer sb = new StringBuffer("Waring: SkillId '" + skillId);
					sb.append("' の グラフィック[");
					sb.append(_attacker.getGfxId() + "] の ActionId:" + skill.getActionId());
					sb.append(" がspr_actionsテーブルに存在しません。");
	...
	private boolean physicalAttack(L1MobSkill skill, L1Character target) {
		L1PhysicalAttack l1pa = PhysicalSkillsTable.getInstance().findBySkillId(skill.getSkillId());
		if (l1pa != null) {
			L1NpcSkillExecutor l1nse = new L1NpcSkillExecutor(_attacker, target, l1pa);
			if (l1nse.isPhysicalAttackSkill()) {
				if (l1nse.usePhysicalAttack()) {
					if (l1pa.getActionId() > 0) {
						try {
							_sleepTime = SprTable.getInstance().getActionList(_attacker.getGfxId()).get(l1pa.getActionId());
						} catch (NullPointerException npe) {
							StringBuffer sb = new StringBuffer("Waring: SkillNo '" + skill.getActionNo());
							sb.append("' の グラフィック[");
							sb.append(_attacker.getGfxId() + "] の ActionId:" + l1pa.getActionId());
							sb.append(" がspr_actionsテーブルに存在しません。");
							System.out.println(sb.toString());
		...
		L1Skill l1skills = SkillTable.getInstance().findBySkillId(skill.getSkillId());
		...
		_attacker.setHeading(_attacker.targetDirection(target.getX(), target.getY())); // 向きのセット

		if (areaHeight > 0) {
			// 範囲攻撃
			ArrayList<L1Object> objs = L1World.getInstance().getVisibleBoxObjects(_attacker,
		...
		if (skill.getActId() > 0) {
			try {
				_sleepTime = SprTable.getInstance().getActionList(_attacker.getGfxId()).get(skill.getActId());
			} catch (NullPointerException npe) {
				StringBuffer sb = new StringBuffer("Waring: SkillId '" + skill.getActionNo());
				sb.append("' の グラフィック[");
				sb.append(_attacker.getGfxId() + "] の ActionId:" + skill.getActId());
				sb.append(" がspr_actionsテーブルに存在しません。");
				System.out.println(sb.toString());
				_sleepTime = _attacker.getAtkSpeed();
			}
			int sleepTime = SprTable.getInstance().getActionList(_attacker.getGfxId()).get(skill.getActId());
			_sleepTime = sleepTime;
		} else if (l1skills != null && l1skills.getRemoveDelay() != 0) {
			_sleepTime = l1skills.getRemoveDelay();
		} else {
			_sleepTime = _attacker.getAtkSpeed();
		}
		return true;
