...
public class L1SpawnUtil {
	...
	public static void dragonPortalSpawn(L1PcInstance pc, int npcId, int mapId,
			int timeMillisToDelete) {
		try {
			L1NpcInstance npc = NpcTable.getInstance().newNpcInstance(npcId);
			npc.setId(IdFactory.getInstance().nextId());
			...
			if (91051 <= npc.getNpcId() && npc.getNpcId() <= 91054) { // ドラゴンポータル(各種)
				npc.setDragonRaidMapId(mapId);
				DragonPortalController.getInstance().getDragonRaid(mapId).setDragonPortal(npc);
				DragonPortalController.getInstance().getDragonRaid(mapId).setPortalOpen(true);
			}
